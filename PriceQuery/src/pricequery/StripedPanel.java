...
public class StripedPanel extends JPanel{
	...
	public void paintComponent(Graphics g){
		...
		for(int i = 0; i < nStripes; i++){
			...
			if(axis == Y_AXIS){
				g2d.fill(new Rectangle2D.Double((getWidth() / (float) nStripes) * i, 0, getWidth() / (float) nStripes, getHeight()));
			}
			else if (axis == X_AXIS){
				g2d.fill(new Rectangle2D.Double(0, getHeight() / (float) nStripes * i, getWidth(), getHeight() / (float) nStripes));
			}
		}
	}
